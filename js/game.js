...
export function startGame() {
    ...
    for (var hi = 0; hi < hn.length; hi++) {
        var h = hn[hi];
        var team = TEAMS[h];
        for (var ri2 = 0; ri2 < team.r.length; ri2++) {
            var x = team.r[ri2];
            var erSpiller = (h === holdNavn && ri2 === ridx);
            var r = {
                navn: erSpiller ? G.navn : x.n,
                orig: x.n,
                hold: h,
                holdAbbr: team.abbr,
                holdClr: team.clr,
...
export function genP(e) {
    ...
    if (e.type === "flad") {
        ...
        // Kuperede etaper: Flere bakker op og ned
        var bakker = ri(4, 7);
        var bakkePos = [];
        ...
        for (var k = 0; k <= e.dist; k++) {
            ...
            for (var i = 0; i < bakkePos.length; i++) {
                var afstand = Math.abs(k - bakkePos[i]);
                var bakkeHøjde = ri(200, 450);
                var bakkeBredde = ri(8, 15);
                if (afstand < bakkeBredde) {
                    bakkeEffect += (bakkeBredde - afstand) / bakkeBredde * bakkeHøjde;
                }
            }
            h = 150 + bakkeEffect + ri(-20, 20);
            h = Math.max(80, Math.min(info.mh, h));
            p.push(h);
        }
        ...
        for (var i = 0; i < bjerge; i++) {
            ...
            var opLængde = ri(12, 22); // Længde af opstigning
            var nedLængde = ri(8, 16); // Længde af nedkørsel
            bjergData.push({
                pos: pos,
                højde: ri(1400, 2300),
                opLængde: opLængde,
                nedLængde: nedLængde,
                opStart: pos - opLængde,
                topPos: pos,
        ...
        if (bjergFinish) {
            var finishPos = e.dist;
            var opLen = ri(15, 25);
            bjergData.push({
                pos: finishPos,
                højde: ri(1600, 2400),
                opLængde: opLen,
                nedLængde: 0, // Ingen nedkørsel - finish på toppen
                opStart: finishPos - opLen,
                topPos: finishPos,
                nedSlut: finishPos
            });
        }
        ...
        e.bjergToppe = [];
        for (var i = 0; i < bjergData.length; i++) {
            if (bjergData[i].topPos < e.dist) { // Ikke finish
...
export function genMP(e) {
    ...
    if (e.type === "bjerg") {
        ...
        // Find højeste punkt på etapen for bjergspurt
        var højestePunkt = 0;
        var højesteKm = Math.floor(e.dist * 0.6);
        // Dette sættes korrekt efter profilen er genereret
        mps.push({
            km: højesteKm,
            type: "bjerg",
            done: false,
        ...
        mps.push({
            km: Math.floor(e.dist * 0.35),
            type: "sprint",
            done: false,
            pts: [20, 17, 15, 13, 11, 9, 7, 6, 5, 4, 3, 2, 1]
        });
        mps.push({
            km: Math.floor(e.dist * 0.65),
            type: "sprint",
            done: false,
            pts: [20, 17, 15, 13, 11, 9, 7, 6, 5, 4, 3, 2, 1]
        });
        ...
        if (felt && felt.ryt.length > 8) {
            ...
            if (faldne.length > 0) {
                ...
                if (bagudGrp) {
                    ...
                    if (faldne.length >= 2) {
                        G.msg = "🏔️ " + faldne.length + " kan ikke følge tempoet!";
                    } else {
                        G.msg = "🏔️ " + faldne[0].navn.split(" ").pop() + " sættes!";
                    }
                }
            }
        }
...
return;
    ...
    if (erPaaStig && G.km > e.dist - 25) {
        ...
        if (felt && felt.ryt.length > 5) {
            ...
            for (var i = 0; i < felt.ryt.length; i++) {
                var r = felt.ryt[i];
                if (r.gc > 84 && !r.ude && !r.sp && r.energi > 40) {
...
export function favoritAngrebKuperet() {
    ...
    for (var i = 0; i < felt.ryt.length; i++) {
        var r = felt.ryt[i];
        if (!r.sp && !r.ude && r.gc > 80) {
            var energiTilMaal = kmTilMaal * 0.9;
            if (r.energi > energiTilMaal + 15) {
                favoritter.push(r);
            }
        }
    }
    ...
    while (fusioneret) {
        ...
        for (var i = 0; i < G.grp.length - 1; i++) {
            ...
            if (afstand >= 0 && afstand < 0.15) {
                // Slå sammen - behold den forreste gruppes position
                for (var j = 0; j < g2.ryt.length; j++) {
                    if (g1.ryt.indexOf(g2.ryt[j]) < 0) {
                ...
                if (g1.type === "felt" || g2.type === "felt") {
                    g1.type = "felt";
                    g1.navn = "Feltet";
                    G.msg = "🔄 Grupper samlet med feltet!";
                } else if (g1.type === "udbrud" && g2.type === "udbrud") {
                    g1.navn = "Udbrud (" + g1.ryt.length + ")";
                    G.msg = "🔄 Udbrud slået sammen!";
                } else if (g1.type === "gruppetto" || g2.type === "gruppetto") {
                    g1.type = "gruppetto";
                    g1.navn = "Gruppetto";
                }
...
export function chkMP() {
    ...
    for (var mi = 0; mi < G.mps.length; mi++) {
        ...
        for (var i = 0; i < pointModtagere; i++) {
            var pts = mp.pts[i];
            if (mp.type === "sprint") {
                G.sprPts[alleRyttere[i].r.navn] = (G.sprPts[alleRyttere[i].r.navn] || 0) + pts;
            } else {
...
if (e.type === "bjerg") {
    ...
    if (kand.length < n) {
        ...
        for (var i = 0; i < felt.ryt.length; i++) {
            var r = felt.ryt[i];
            if (!r.sp && !r.ude && r.energi > 50 && kand.indexOf(r) < 0) {
                ekstra.push(r);
            }
        }
...
export function accUdb() {
    ...
    if (reagerende.length > 0) {
        ...
        G.msg = "🚀 Du angriber med " + d.med.length + " andre!";
    } else {
        G.msg = "🚀 SOLO ANGREB!";
    }
        ...
        if (tidsForskel > 400 && Math.random() < 0.02) {
            ...
            if (afstand < 0.08) {
                for (var j = 0; j < g.ryt.length; j++) {
                    if (felt.ryt.indexOf(g.ryt[j]) < 0) {
                        felt.ryt.push(g.ryt[j]);
                        g.ryt[j].gi = 0;
                    }
                }
                G.msg = "🏳️ Udbrud indhentet!";
                continue;
            }
        }
        newGrp.push(g);
        ...
        for (var i = 1; i < gruppettoer.length; i++) {
            if (gruppettoer[0].pos - gruppettoer[i].pos < 1) {
                for (var j = 0; j < gruppettoer[i].ryt.length; j++) {
                    gruppettoer[0].ryt.push(gruppettoer[i].ryt[j]);
                }
                gruppettoer[i].ryt = [];
            }
        }
...
if (reagerende.length > 0) {
    ...
    G.msg = "🚀 SOLO ANGREB!";
        ...
        if (grpSize === 1) {
            ...
            var andresEnergi = 0;
            for (var i = 0; i < sg.ryt.length; i++) {
                if (sg.ryt[i] !== G.spiller) {
                    andresEnergi += sg.ryt[i].energi;
                }
            }
...
if (Math.random() < ch) {
    ...
    if (reagerende.length > 0) {
        ...
        G.msg = "🚀 DU SLIPPER VÆK! Solo angreb!";
                ...
                var beskeder = [
                    "❌ De holder dit hjul!",
                    "❌ Lukket ned af gruppen!",
                    "❌ De vil ikke slippe dig!",
                    "❌ Angrebet neutraliseret!",
                    "❌ De matcher dit tempo!"
                ];
                G.msg = beskeder[ri(0, beskeder.length - 1)];
            }
        }
        ...
        if (foran) {
            ...
            if (Math.random() < ch) {
                ...
                sg.pos += 0.3;
                G.msg = "❌ Kan ikke nå dem... kæmper videre!";
            }
        } else {
            sg.pos += 0.3;
            G.msg = "💨 Kæmper videre alene...";
        }
    }
    rend();
}
...
export function rend() {
    ...
    if (G.vent && G.udata) {
        var d = G.udata;
        var medTxt = d.med.length > 0 ? d.med.length + " vil med!" : "Solo?";
        var dis = r.energi < d.cost ? " disabled" : "";
        mod = '<div class="modal"><div class="mbox">' +
            '<h2 style="color:#ff0">⚡ UDBRUD?</h2>' +
            '<p>' + medTxt + '</p><p class="warn">Koster: ' + d.cost + '%</p>' +
            '<p>Din energi: ' + Math.round(r.energi) + '%</p>' +
            '<div style="margin-top:12px">' +
            '<button class="btn btn-big btn-y" onclick="accUdb()"' + dis + '>[U] HOP MED!</button>' +
...
if (g.ryt.length === 1) {
    ...
    if (g.type === "felt") {
        ...
        grpNavn = g.navn; // Behold evt. navn (Udbrud, Favoritgruppe, osv.)
    } else {
...
if (her) {
    ...
    if (harGul) col = "#ff0";
        ...
        if (her) {
            ctx.beginPath();
            ctx.arc(gx, gy - 16, rad + 8, 0, Math.PI * 2);
    ...
    ctx.font = "11px monospace";
    ctx.textAlign = "right";
    ctx.fillText(Math.round(maxH) + "m", l - 5, 20);
    ctx.fillText(Math.round(minH) + "m", l - 5, 12 + h);
    ctx.textAlign = "center";
    ctx.fillText("0", l, 12 + h + 18);
    ctx.fillText(e.dist + " km", l + w, 12 + h + 18);
}
...
export function startTT() {
    ...
    for (var i = 0; i < G.ryttere.length; i++) {
        ...
        while (r.energi > 20) {
            ...
            var accChance = (r.tt / 100) * (r.energi / 100) * 0.5;
...
export function afslutTT() {
    ...
    for (var i = 0; i < G.ryttere.length; i++) {
        var rest = G.ryttere[i].re / 100 * 30 + 15;
...
export function afslut() {
    ...
    var tidsProcent;
    if (e.type === "bjerg") {
        tidsProcent = 0.24; // 24% af vindertid
    } else if (e.type === "kuperet") {
        tidsProcent = 0.22; // 20 af vindertid
    } else if (e.type === "tt") {
        tidsProcent = 0.30; // 30% af vindertid (enkeltstart)
    } else {
    ...
    for (var i = 0; i < res.length; i++) {
        var x = res[i];
        var bagud = x.tid - vinderTid;
        if (bagud > tidsGrænse && !x.r.ude) {
            x.r.ude = true;
            udeRyttere.push(x.r);
        }
    }
...
export function visRes(res, erTT, udeRyttere, vinderTid, tidsGrænse) {
    ...
    if (udeRyttere && udeRyttere.length > 0) {
        ...
        for (var i = 0; i < udeRyttere.length; i++) {
            var r = udeRyttere[i];
            // Find deres tid
            var rTid = 0;
            ...
            html += '<div class="row" style="color:#f44"><span>' + r.navn + ' (' + r.holdAbbr + ')</span><span>+' + ft(rTid) + '</span></div>';
        }
